...
const checkBoard = (board, words) => {
    ...
    for (row of board) {
        ...
        found += (words.includes(row.join("")) ? 1 : 0) + (words.includes(reversed.join("")) ? 1 : 0) - (words.includes(row.join("")) && words.includes(reversed.join("")) && row.join("") == reversed.join("") ? 1 : 0);
    }
    ...
    for (let c = 0; c < board.length; c++) {
        ...
        found += (words.includes(word) ? 1 : 0) + (words.includes(revWord) ? 1 : 0) - (words.includes(word) && words.includes(revWord) && word == revWord ? 1 : 0);
    }
